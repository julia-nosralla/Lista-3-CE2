...
```{r}
lista_pokemon <- data.frame(pokemonJSON1)
View(lista_pokemon)
```

Aqui, ao tentarmos visualizar os dados como dataframe, obtemos um erro, referente a uma diferença na quantidade de linhas. Se explorarmos qualquer um dos objetos `pokemonJSON` usando a função `View()`, notaremos que o que nos interessa está localizado dentro de `results`.

```{r}
View(pokemonJSON1)
```

Dessa forma, precisamos alterar o endpoint.

```{r}
lista_pokemon <- data.frame(pokemonJSON1$results)
View(lista_pokemon)
```

### Acessar os dados de dentro das APIs do dataframe
